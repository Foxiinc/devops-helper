...
- SSH terminal with xterm.js (WebGL renderer, PTY resize, session tabs)
- Terminal clipboard: **Ctrl+Shift+C/V**, **Ctrl+Insert** / **Shift+Insert** (WebView2 accelerator keys disabled so copy doesn’t open DevTools)
- Server list with **folders**, password / key authentication
- Known host verification with trust dialog
- ED25519 key generation, import from `~/.ssh`, ssh-copy-id
- Dual-panel SFTP file browser (local Windows FS + remote), **recursive folder upload/download**
...
- **Monitor** — host processes (ps/proc), Docker containers, trusted binary registry (SHA256), **sortable columns**, filter
- **Updates** — apt/dnf upgradable packages, **run upgrade** (all or selected), CVE hints via OSV.dev (cached 24h)
- **Activity log** — JSON-lines audit of API calls and UI actions (passwords redacted), Settings → Navigation

## Security & storage

- Private keys and passwords encrypted with **AES-256-GCM**
- Master key stored in **`%APPDATA%\com.briskbastion.app\master.key`** (primary); migrates from Windows Credential Manager on first launch
- If the master key is lost but the DB still has secrets, the app starts with a notice — re-enter server passwords once (no startup panic)
- Activity log: `%APPDATA%\com.briskbastion.app\activity.log`
...
| Vite `EBUSY` on `target\...\*.dll` | Stop parallel `cargo build` / use `npm run tauri dev`; `vite.config.ts` ignores `target/` |
...
### `crypto error: aead::Error` / «Encryption key file is missing»

Passwords and SSH private keys are encrypted with a **master key** file:

`%APPDATA%\com.briskbastion.app\master.key`

This error means stored secrets cannot be decrypted — the key was deleted, the DB was copied from another machine, or Credential Manager was reset before migration.
...
3. If the app shows a **vault reset notice** on startup, dismiss it after re-entering passwords for affected servers

**Backup tip:** keep `master.key` next to your app data folder when moving machines.

Do **not** delete `master.key` unless you intend to re-enter all credentials.
...
- [x] Server list + key vault (ED25519, AES-GCM)
- [x] SFTP browser + rsync-like sync
- [x] `.bastion` scenario runner (`local` / `remote` / `sync`)
- [x] Tauri 2 desktop shell (Windows)

### v0.1.1 — UX polish (done)

| ID | Задача | Модуль | Статус |
|----|--------|--------|--------|
| P1 | Папки серверов в sidebar (create / rename / delete) | `core/src/db/`, UI | [x] |
| P2 | SFTP: рекурсивный upload/download папок | `core/src/sftp/`, `SftpPanel.tsx` | [x] |
| P3 | Терминал: copy/paste hotkeys, fix WebView2 DevTools на Ctrl+Shift+C | `TerminalView.tsx`, `tauri-plugin-prevent-default` | [x] |
| P4 | Activity log (JSON-lines, redact passwords) + просмотр в Settings | `core/src/audit.rs`, `ActivityLogSection.tsx` | [x] |
| P5 | Vault: `master.key` в app data, миграция из Credential Manager, notice при reset | `core/src/crypto.rs` | [x] |
| P6 | Monitor: сортировка колонок (host), sort/filter для Docker | `MonitorPanel.tsx` | [x] |
...
| M1 | UI: таблица процессов, авто-refresh, фильтр, **сортировка по колонкам** | `src/components/MonitorPanel.tsx` | [x] |
...
| M3 | UI: дерево контейнеров, статус, CPU/RAM, expand → processes, **sort + filter** | MonitorPanel | [x] |
...
| U1 | UI: Package \| Installed \| Available \| Size, Refresh, **Upgrade all / selected** | `UpdatesPanel.tsx` | [x] |
| U1 | Tauri: `run_updates(server_id, packages?)` — `apt-get upgrade` / `dnf upgrade` | `core/src/updates/`, `src-tauri/` | [x] |

**Принцип:** апгрейд только по явному действию пользователя (кнопка + confirm); dry-run preview — в backlog.
...
core/src/
  ...
  updates/          # U1–U3: package diff, OSV client, cache
    ...
    dnf.rs
    osv.rs
    upgrade.rs      # apt/dnf upgrade commands
  audit.rs          # activity.log (JSON-lines)
  crypto.rs         # master.key vault
src/
  components/
    MonitorPanel.tsx
    UpdatesPanel.tsx
...
1. Берём **следующую незакрытую задачу** (сейчас: **U2 «fix available?»**, **M4 Docker API**, или **v0.6+ backlog**).
